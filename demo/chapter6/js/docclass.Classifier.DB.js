...
DocClass.prototype.Classifier.prototype.catcount = function(cat, callback) {
  ...
  this.db.transaction(
    function(tx) {
      tx.executeSql('select count from cc where category=?', [cat],
        function(rt, rs) {
          if (rs.rows.length === 0) {
            res = 0;
          }
...
DocClass.prototype.Classifier.prototype.categories = function(callback) {
  ...
  this.db.transaction(
    function(tx) {
      tx.executeSql('select category from cc', [],
        function(rt, rs) {
          if (rs.rows.length === 0) {
            res = [];
          }
...
DocClass.prototype.Classifier.prototype.totalcount = function() {
  this.db.transaction(
    function(tx) {
      tx.executeSql('select sum(count from cc', [],
        function(rt, rs) {
          if (rs.rows.length === 0) {
            return 0;
          }
...
DocClass.prototype.Classifier.prototype.train = function(item, cat, callback) {
  ...
  var self = this, f = Object.keys(features), n = f.length;
